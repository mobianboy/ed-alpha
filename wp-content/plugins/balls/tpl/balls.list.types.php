...
<div class="left" id="post-type-templates">
	...
	<p class="description">
		<? _e('Select the corresponding post type template, and then place/edit your desired widgets settings/position.') ?>
	</p>
	<ul>
<? if(count($post_types)): ?>
      ...
      <? if(count($templates)): ?>
          ...
          <? if(count($templates)): ?>
			      <? foreach($templates as $key => $template): ?>
				      <li>
					      <a href="admin.php?page=balls_admin&action=position&template_id=<?= $template->id ?>" class="button">
