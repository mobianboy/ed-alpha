...
// Get songs by the artist in the profile
$artist_songs = song::get_artist_songs($user->ID);
...
<div class="portlet songs">
	<div class="overlay">
		<div class="caption">
			<h3>Songs</h3>
			<div class="number">(<?= count($artist_songs) ?>)</div>
		</div>
	</div>
  <div class="explode">
    <h2><span>Songs by: </span><?= $user->display_name ?></h2>
    <ul>
  <? if(count($artist_songs)): ?>
    <? foreach($artist_songs as $k => $song): ?>
