...
jQuery.fn.bullseye = function (settings, viewport) {
    settings = jQuery.extend({ offsetTop: 0, offsetHeight: 0, extendDown: false }, settings);
    ...
    return this.each(function () {
        var $element = $(this),
            ...
            if ( (scrollBottom < y1 || (settings.extendDown ? false : scrollTop > y2)) ||  (scrollRight < x1 || scrollRight > x2) )
                outsideViewport();
            else
                insideViewport();
        };
    ...
    function isFocused($element) { return $element.data(isFocusedKey); }
    function setFocused($element) { $element.data(isFocusedKey, true); }
    function clearFocused($element) { $element.data(isFocusedKey, false); }
};
