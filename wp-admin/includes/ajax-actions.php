...
function wp_ajax_delete_meta() {
	...
	if ( delete_meta( $meta->meta_id ) ) {
	  // 2013-05-07 Steven Kornblum hacking wordpress to push news images through to Image proc (mfp)
    $post = get_post($meta->post_id);
	  if($post->post_type == 'post' && $meta->meta_key == 'thumbnail_image') {
	    $mfp_hack = array();
	    $mfp_hack['url'] = 'http://'.MFP_HOST.'/delete';
	    $mfp_hack['data'] = array(
	      'type'    => 'news',
	      'id'      => $meta->post_id,
	      'format'  => 'image',
	    );
	    $mfp_hack['json']['data'] = json_encode($mfp_hack['data'], TRUE);
	    $mfp_hack['ch'] = curl_init();
	    curl_setopt($mfp_hack['ch'], CURLOPT_URL, $mfp_hack['url']);
	    curl_setopt($mfp_hack['ch'], CURLOPT_POST, count($mfp_hack['json']));
	    curl_setopt($mfp_hack['ch'], CURLOPT_POSTFIELDS, $mfp_hack['json']);
	    //curl_setopt($mfp_hack['ch'], CURLOPT_VERBOSE, 1);
	    curl_setopt($mfp_hack['ch'], CURLOPT_RETURNTRANSFER, TRUE);
	    $mfp_hack['res'] = json_decode(curl_exec($mfp_hack['ch']), TRUE);
	    curl_close($mfp_hack['ch']);
	    if($mfp_hack['res']['status']['code'] != 1) {
	      echo '<pre>News Image error... please try again or notify Tech dept.</pre>';
	      exit;
	    }
	  } // End mfp hack! ;-)
		wp_die( 1 );
...
function wp_ajax_add_meta() {
  ...
  if($post->post_type == 'post' && $meta->meta_key == 'thumbnail_image') {
    $mfp_hack = array();
    $mfp_hack['url'] = 'http://'.MFP_HOST.'/upload';
    $mfp_hack['data'] = array(
      'type'    => 'news',
      'id'      => $meta->post_id,
      'url'     => $meta->meta_value,
    ...
    $mfp_hack['res'] = json_decode(curl_exec($mfp_hack['ch']), TRUE);
    curl_close($mfp_hack['ch']);
    if($mfp_hack['res']['status']['code'] != 20) {
      echo '<pre>News Image error... please try again or notify Tech dept.</pre>';
      exit;
    }
  } // End mfp hack! ;-)
